...
function RepoSection({ repoStatus, repoMeta, unpushed, onPush }: {
  ...
  return (
    <div style={styles.repoRoot}>
      {/* Repo header */}
      <div
        style={{ ...styles.repoHeader(repoColor), background: hovered ? `${repoColor}20` : `${repoColor}14` }}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
      ...
      >
        ...
        {!hasUpstream && (
          <span style={styles.noUpstreamBadge}>no upstream</span>
        )}
        {hovered && hasUpstream && ahead > 0 && (
          <button
            style={styles.pushBtn}
            title={`Push ${ahead} commit${ahead === 1 ? '' : 's'}`}
            onClick={e => { e.stopPropagation(); onPush(repoStatus.repoId); }}
          >
        ...
        <div style={styles.repoBody}>
          {!hasUpstream ? (
            <div style={styles.warningRow}>
              <Codicon name="warning" style={{ marginRight: '6px', flexShrink: 0 }} />
              No upstream branch configured for <strong>{repoStatus.branch.name}</strong>
            </div>
          ) : ahead === 0 ? (
            ...
            <div style={styles.loadingRow}>No unpushed commits found</div>
          )}
        </div>
      )}
    </div>
...
export function PushTab({ repos, repoMetas, unpushedMap, onPush, onPushAll }: Props) {
...
const styles = {
  ...
  noUpstreamBadge: {
    ...
    borderRadius: '3px', padding: '1px 5px', fontSize: '10px', flexShrink: 0,
  } as React.CSSProperties,
  pushBtn: {
    display: 'flex', alignItems: 'center',
    background: 'var(--vscode-button-background)', color: 'var(--vscode-button-foreground)',
    border: 'none', borderRadius: '3px', padding: '2px 8px',
    cursor: 'pointer', fontSize: '11px', fontFamily: 'var(--vscode-font-family)', flexShrink: 0,
  } as React.CSSProperties,
  repoBody: {
    background: 'var(--vscode-sideBar-background)',
  } as React.CSSProperties,
  warningRow: {
    display: 'flex', alignItems: 'center', padding: '8px 12px', fontSize: '11px',
    color: 'var(--vscode-editorWarning-foreground, #e9ae00)',
  } as React.CSSProperties,
  upToDate: {
  ...
  commitRow: {
    display: 'grid', gridTemplateColumns: '44px 1fr', gridTemplateRows: 'auto auto',
    gap: '0 8px', padding: '5px 12px', cursor: 'default',
  } as React.CSSProperties,
  commitHash: {
  ...
  commitMessage: {
    fontSize: '12px', gridRow: '1', gridColumn: '2',
    overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const,
  } as React.CSSProperties,
  commitMeta: {
